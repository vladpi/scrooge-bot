...
async def report(
    to_chat_id: int, report: ReportSchema, message_for_update: Optional[Message] = None,
):
    ...
    day_period_button = InlineKeyboardButton('За день', callback_data=reports_cb.new(period=ReportPeriod.DAY))
    week_period_button = InlineKeyboardButton('За неделю', callback_data=reports_cb.new(period=ReportPeriod.WEEK))
    month_period_button = InlineKeyboardButton('За месяц', callback_data=reports_cb.new(period=ReportPeriod.MONTH))
